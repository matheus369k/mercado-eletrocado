...
export const ProductCard = ({ data, quantity }: SliceProductCartType) => {
  ...
  return (
    <ProductsLessProvider>
      <div aria-label={data.model} className={styles.cart_product_card}>
        {quantity > 1 && <span className={styles.card_quantity}>{quantity}X</span>}
        <img
          aria-label="cart product picture"
          onClick={() => handleRedirectionToProduct(data._id)}
          src={data.img}
          alt={data.model}
        />
        <div className={styles.card_content}>
          ...
          <h3>{data.model}</h3>
          <LessProductControls _id={data._id} />
        </div>
      </div>
    </ProductsLessProvider>
  );
